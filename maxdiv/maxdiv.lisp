        ...
        (let* ((vix (heap-peek))
          ...
          (if (= (* r divw) w)
              (progn
                ...
                (return-from find-max-divisor divw))
              (if (> (* divw divw) w)
  ...
  (let ((ops0 *ops*)
    ...
    (do ((i (1- n) (1- i)))
      ...
      (do ((divw (aref v i))
        ...
        (let* ((w (aref v j))
               (r (floor w divw)))
          ; (format t "~a~%" (list :i i :j j :w w :divw divw :r r))
          (when (= (* r divw) w)
            (format t "~a divides ~a, ops: ~a~%" divw w (- *ops* ops0))
            (return-from find-max-divisor-bf divw))))))
  nil)
...
(defun test-maxdiv (n maxw)
  (let ((v (make-array n)))
    ...
    (format t "optimized~%")
    (find-max-divisor v)
    (format t "brute force~%")
    (find-max-divisor-bf v)))
